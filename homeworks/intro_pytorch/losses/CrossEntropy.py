...
class CrossEntropyLossLayer(nn.Module):
    ...
    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        ...
        """
        prob = y_pred[torch.arange(y_pred.shape[0]), y_true] #the torch.arange functions allows to iterate through all the row indices of the predictions and retrieve the predicted probabilities for the true class
        log_prob = -torch.log(prob) #computing negative log of each element in the tensor vector b (similar to np.log)
        cross_entropy = torch.mean(log_prob) #now we compute the mean over all sample considered for predictions and returning it
