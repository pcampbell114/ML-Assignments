...
class SGDOptimizer(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr: float) -> None:
        ...
        self.lr = lr #initializing the learning rate
        self.params = params #initializing the model parameters
    ...
    def step(self, closure=None):  # noqa: E251
        ...
        """
        for group in self.param_groups: #we must iterate through each of the parameter groups where we apply GD to the parameters in each individual group
            for p in group['params']: #now for each parameter in the group we iterate through
                 p.data -= self.lr*p.grad.data #we perform gradient descent using the standard formula and decrementation to update parameter values using .grad.data from the gradients computed in the backward() step
        return None
        raise NotImplementedError("Your Code Goes Here")
